...
public class CompletionZone : MonoBehaviour
{
    ...
    [Header("Stair Puzzle Check")]
    public StairPuzzleManager stairPuzzleManager;
    ...
    private void TryStartPortal()
    {
        ...
        if (stairPuzzleManager != null && !stairPuzzleManager.IsPuzzleReadyForPortal())
        {
            if (hintText != null)
            {
                hintText.text = "The portal is locked. Complete the stair sequence first.";
            }

            Debug.Log("Portal locked. Stair puzzle sequence is not finished.");
            return;
        }
    ...
    private IEnumerator PortalAnimation()
    {
        ...
        // Move players to portal target spots first
        ...
        while (alignElapsed < alignDuration)
        {
            ...
            player2DTransform.position = Vector3.Lerp(
                start2DPos,
                player2DTarget.transform.position,
                progress
            );

            playerTDTransform.position = Vector3.Lerp(
                startTDPos,
                playerTDTarget.transform.position,
                progress
            );
        ...
        Quaternion start2DRot = player2DTransform.rotation;
        Quaternion startTDRot = playerTDTransform.rotation;
        ...
        // Reset players before moving to next sublevel/victory
        ...
        // Complete stair puzzle sound + score bonus
        if (stairPuzzleManager != null)
        {
            stairPuzzleManager.CompletePuzzle();
        }

        // Move to next sublevel or victory screen
